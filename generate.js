...
function addExports(json) {
  return `module.exports = ${json}`;
}

// toJSON :: { k: v } -> JSON
const toJSON = R.compose(addExports, js => JSON.stringify(js, null, 2));
  ...
  try {
    if (isTachyonsModule(module)) {
      return 'node_modules/' +
        module +
        '/' +
        require('./node_modules/' + module + '/package.json').style;
  ...
  return R.reduce(
    (acc, key) => {
      if (isHoverStyle(key)) {
        ...
        if (styles[rootKey]) {
          return R.compose(R.dissoc(key), R.assoc(':hover', styles[key]))(acc);
        }
